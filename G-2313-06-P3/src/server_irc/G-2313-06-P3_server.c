#include "../../includes/server_irc/G-2313-06-P3_server.h"
...
* <code>
* \b #include \b <G-2313-06-P3_server.h>
* </code>
* @section variables Variables definidas
* <p>Se utilizan las siguientes variables definidas en G-2313-06-P3_server.h
* <p><code>
...
int server_start(){
    ...
    if(server_ssl==false){
      ...
      Direccion.sin_port=htons(port); /* Asignar el puerto */
      Direccion.sin_addr.s_addr=htonl(INADDR_ANY); /* Aceptar todas las IPs */
      bzero((void *)&(Direccion.sin_zero), 8);

      syslog(LOG_INFO, "Socket conectado en puerto %d", port);
      ...
      ssl_ctx = inicializar_nivel_SSL(&socket_desc);
      if(fijar_contexto_SSL(ssl_ctx, ssl_cert_1, ssl_cert_2)==1){
        syslog (LOG_INFO, "Fijar devuelve 1");
      } else {
        syslog (LOG_INFO, "Fijar devuelve 0");
      }
    }
    return socket_desc;
}
  ...
  char buffer[8096];
    ...
    if(server_ssl){
      ...
      client_socket = SSL_get_fd(ssl);
		...
		if (client_socket < 0){
		    syslog(LOG_ERR, "ERROR aceptando conexiones %d", client_socket);
        exit(EXIT_FAILURE);
		}
    ...
    t = task_create();
    task_init(t, server_start_communication, ssl);
    task_add(pool, t);
    // while(1){
  	// 	bzero(buffer, sizeof(char)*8096);
  	// 	state = recibir_datos_SSL(ssl, &buffer[0]);
  	// 	if(state>0){
    //     syslog(LOG_ERR, "Recibo: %s", buffer);
    //   }
    // }
	}
...
* </ul>
*/
void *server_start_communication(SSL *ssl_arg){
  int val_read, *register_status = malloc(sizeof(int));
  char str[8096], *command=NULL, *unpipeline_response=NULL, *nick = malloc(sizeof(char) * 10);
  int socket_desc =  SSL_get_fd(ssl_arg), state = 0;
  ...
  while(server_status)
  {
    if(server_ssl){
      ...
      if(server_check_socket_status(socket_desc)){
        syslog (LOG_INFO, "[DESC: %d] [NO SSL] Detectado ausente, desconectando...", socket_desc);
        break;
      }
      memset(str, 0, 8096);
      val_read = recv(socket_desc, str, 8096, 0);
      if((val_read<1)||(str[0]=='\0')){
        syslog (LOG_INFO, "[DESC: %d] [NO SSL] Detectado ausente, desconectando...", socket_desc);
        break;
      }
    }
    syslog (LOG_INFO, "Recibo: %s", str);
    ...
    server_execute_function(IRC_CommandQuery(command), command, socket_desc, ssl_arg, nick, register_status);
    if(command){
      free(command);
      command = NULL;
    }
...
void server_execute_function(long functionName, char* command, int desc, SSL *ssl, char* nick, int* register_status){
  ...
  if ((functionName<0)||(functionName>IRC_MAX_USER_COMMANDS)||(functions[functionName]==NULL)){
    ...
    (*functions[functionName])(command, desc, ssl, nick, register_status);
  }
}
  ...
  if(argc>0){
    for(i = 0; i<argc; i++){
      if(strcmp(argv[i], "--h")==0){
        daemon=0;
      } else if((strcmp(argv[i], "--ssl")==0)||(strcmp(argv[i], "--ssl")==0)){
        server_ssl=true;
        syslog(LOG_ERR, "[SERVIDOR IRC] ES SSL");
      } else if(strcmp(argv[i], "--port")==0){
        if((i+1)<=argc){
          port = atoi(argv[i+1]);
          syslog(LOG_ERR, "[SERVIDOR IRC] PUERTO: %d", port);
        }
      }
    }
  }
